...
void Timer::end(const char *msg, const char *dstFn)
{
    ofstream outStrm;
    ...
    if(tm_elaps->tm_yday != 0.0f)
    {
        outStrm<<" "<<tm_elaps->tm_yday<<" day(s)";
    }

    if(tm_elaps->tm_hour != 0.0f)
    {
        outStrm<<"  "<<tm_elaps->tm_hour<<" hour(s)";
    }
    if(tm_elaps->tm_min != 0.0f)
    {
        outStrm<<"  "<< tm_elaps->tm_min<<" minute(s)";
    }
    if(tm_elaps->tm_sec != 0.0f)
    {
        outStrm<<"  "<<tm_elaps->tm_sec<<" second(s)";
    }
    outStrm<<endl;
    free(tm_elaps);
    outStrm.close();
